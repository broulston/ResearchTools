...
def _to_rgb(c):
    ...
    if (len(c) == 1) or isinstance(c, str):  # if1: if c is a single element (number of string)
        ...
        try:   # try1: check if c is numberic or not
            ...
            # try1: if not numerics is not (only) RGB or RGBA colors
            # convert the list/tuble/array of colors
            # into a list of numpy arrays of RGB
            ...
            if len(arrcsh) == 1:  # i f2: if 1D array given
                if(arrcsh[0] == 3 or arrcsh[0] == 4):
                    # if3: if RGB or RBGA
                    rgbs = [np.array(cC.to_rgb(c)), ]
                    # list with 1 RGB numpy array
